 ...
 */
export const typeFilter = format => format === 'int' || format === 'double' || format === 'number' || format === 'timestamp';
 ...
 * @param {Object} data 原始数据
 */
export const getDataValue = (field, data) => objGet(data, field);
 ...
 */
export const realName = (name) => {
  const baseName = _basePath(name);
  if (typeof baseName === 'object' && Array.isArray(baseName) && baseName.length > 1) {
    const realname = baseName.reduce((a, b) => a += `#${b}`, '_formdata_');
    return realname;
  }
  return baseName[0] || name;
};
...
export const objGet = (object, path, defaultVal = 'undefined') => {
  ...
  // 递归处理，返回最后结果
  const val = newPath.reduce((o, k) => (o || {})[k], object);
