...
export default {
  ...
  methods: {
	...
	*/
    chooseFiles() {
      const _extname = getExtName(this.fileExtname);
      // 获取后缀
      // eslint-disable-next-line no-undef
      uniCloud
        .chooseAndUploadFile({
	...
	*/
    async chooseFileCallback(res) {
      const _extname = getExtName(this.fileExtname);
      const isOne = (Number(this.limitLength) === 1
						&& this.disablePreview
						&& !this.disabled)
      ...
      for (let i = 0; i < files.length; i++) {
        ...
        const filedata = await getFileData(files[i], this.fileMediatype);
        filedata.progress = 0;
